...
from src.domain.bundles.view_models import (
    backlog_to_dict,
    build_backlog_views,
...
def _get_engine_version() -> str:
    """Get engine version from package or fallback.

    Returns:
        Package version string from importlib.metadata, or DEFAULT_ENGINE_VERSION
...
def default_builder(ctx: EvalContext) -> dict[str, Any]:
    ...
    # Never leave commit as None - use "dev" fallback
    git_commit = engine.get("commit") or "dev"
    engine.setdefault("commit", git_commit if git_commit else "dev")
    engine.setdefault(
        "name",
...
def engineering_backlog_builder(ctx: EvalContext) -> dict[str, Any]:
    ...
    """
    from src.domain.bundles.backlog_config import filter_actions_by_keywords, get_keywords_for_split
