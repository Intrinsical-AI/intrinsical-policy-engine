...
from src.common.cache import cached_medium
...
from src.domain.core.subject_profile import SubjectProfile, build_subject_profile
from src.domain.ports import ContractBundle
from src.domain.services.article_overlay import overlay
from src.domain.services.coherence_checker import (
    compute_flag_coherence_warnings,
    compute_role_warnings,
)
from src.domain.services.dedup_service import DedupResult, dedupe_ids_with_trace
from src.domain.services.derivation_closure import DerivationResult, derive_with_trace
from src.domain.services.duedate_service import apply_due_hints, flatten_calendar, resolve_calendar
from src.domain.services.flag_article_index import build_flag_article_index
from src.domain.services.integrity import compute_bundle_hash, compute_plan_hash
from src.domain.services.metrics import compute_metrics
from src.domain.services.outcome_classifier import (
    classify_outcome_axes,
    classify_outcome_from_axes,
)
from src.domain.services.routing_router import resolve_routing_route
...
from src.domain.services.tracer import build_trace
...
def assess_from_bundle(
    ...
    Example:
        >>> from src.domain.bundles.context_builders import build_bundle_from_path
...
def _get_engine_version() -> str:
    """Get engine version from package metadata or fallback."""
    try:
        from importlib.metadata import version

        return version(CANONICAL_ENGINE_NAME)
    except Exception:  # noqa: BLE001
        # Fallback: try reading from src.__version__ if in development/editable mode
        try:
...
def _get_initial_flags(bundle, raw_answers: dict) -> set[str]:
    ...
    from src.domain.services.questionnaire_engine import eval_questions, sanitize_answers_dict
