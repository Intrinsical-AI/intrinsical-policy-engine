...
def _require_dir(framework_dir: Path, configured_rel: Any, *, label: str) -> Path:
    ...
    configured = framework_dir / configured_rel
...
def resolve_manifest_entries(
    ...
    for entry in entries_list:
        ...
        if any(ch in rel for ch in ("*", "?", "[")):
            matches = sorted(entry_path.parent.glob(entry_path.name))
            if not matches:
                missing.append(rel)
            resolved.extend(matches)
            continue
        if entry_path.is_dir():
            resolved.extend(sorted(entry_path.glob("*.yml")))
            continue
...
def load_framework_layout(framework_dir: Path | str | PathLike[str]) -> FrameworkLayout:  # noqa: C901
    """Build a strict framework layout from a framework-pack root."""
    resolved_framework_dir = Path(framework_dir).resolve()
    manifest_path = resolved_framework_dir / "manifest.yml"
    framework_version_path = resolved_framework_dir / "FRAMEWORK_VERSION.yml"
    if not manifest_path.exists():
        raise FileNotFoundError(f"manifest.yml not found: {manifest_path}")
    if not framework_version_path.exists():
        raise FileNotFoundError(f"FRAMEWORK_VERSION.yml not found: {framework_version_path}")
