...
from src.app.gating.export_gate import evaluate_bundle_coherence
...
class ManifestStrategy(ExportStrategy):
    ...
    def execute(self, exporter: FilesystemExporter, context: ExportContext) -> ArtifactsDelta:
        ...
        # 4c. Copy verify_integrity.py to tools/ (INV-A-80)
        verifier_file = self._copy_verifier_script(exporter, context)
        if verifier_file:
            generated_files.append(verifier_file)
    ...
    def _generate_checksums(  # noqa: C901
        ...
        for p in context.out_dir.rglob("*"):
        ...
        # Check if GPG signing should be skipped (CI/dev environments)
        ...
        try:
            ...
            if skip_gpg:
                ...
                if logger:
                    logger.info("export.gpg.skipped", {"reason": "IPE_SKIP_GPG_SIGNING=1"})
    ...
    def _generate_fingerprint(self, exporter: FilesystemExporter, context: ExportContext) -> None:
        ...
        # Normalize to resolved absolute paths so Path and str inputs dedupe the same file.
        seen: set[str] = set()
        files_to_hash: list[Path] = []

        # Add from generated_files (normalize to resolved absolute)
        for p in context.generated_files:
            resolved = Path(p).resolve()
            rel_path = self._relative_to_out_dir(context, resolved)
            if rel_path and self._is_excluded_from_hashing(rel_path):
                continue
            key = str(resolved)
            if key not in seen:
                seen.add(key)
        ...
        # Also scan output directory for any files we might have missed
        # (e.g., files generated by BundleExporter)
        for p in context.out_dir.rglob("*"):
            if p.is_file() and p.name != FINGERPRINT_JSON and not p.name.startswith("."):
                rel_path = p.relative_to(context.out_dir).as_posix()
    ...
    def _is_excluded_from_hashing(self, rel_path: str) -> bool:
        return any(rel_path.startswith(prefix) for prefix in self._EXCLUDED_HASH_PREFIXES)
    ...
    def _copy_verifier_script(
        self, exporter: FilesystemExporter, context: ExportContext
    ) -> Path | None:
        """Copy verify_integrity.py to tools/ directory.

        Per docs/invariants/ARTEFACTS-v1.md §3.15 (INV-A-80):
        - The verifier is optional but recommended
        - It requires no internet and no heavy dependencies
        - It allows offline verification of the package

        The script is located at project_root/tools/verify_integrity.py.
        """
        import shutil

        logger: StructuredLogger | None = getattr(exporter, "_logger", None)

        # Find the verifier script in the project
        # Strategy: walk up from this file to find project root with tools/
        script_name = "verify_integrity.py"
        candidates = [
            Path(__file__).resolve().parents[5] / "tools" / script_name,  # From strategies/
            Path.cwd() / "tools" / script_name,
        ]
        ...
        if source_script is None:
            if logger:
                logger.debug(
                    "export.verifier.not_found",
                    {"searched": [str(c) for c in candidates]},
                )
            return None

        # Create tools/ directory and copy script
        tools_dir = context.out_dir / "tools"
        ...
        try:
            ...
            if logger:
                logger.info("export.verifier.copied", {"path": str(target_path)})

            return target_path

        except OSError as e:
            if logger:
                logger.warning("export.verifier.copy_failed", {"error": str(e)})
            return None
    ...
    def _generate_actions_json(
        ...
        from src.domain.bundles.view_models import build_backlog_views
