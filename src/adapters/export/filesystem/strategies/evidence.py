...
from src.adapters.export.base.evidence.evidence_quality import (
...
from src.adapters.frameworks.layout_loader import load_framework_layout_from_path_cached
from src.adapters.logging import StructuredLogger
from src.app.config.constants import METADATA_DIR, METRICS_JSON
...
class EvidenceStrategy(ExportStrategy):
    ...
    def execute(self, exporter: FilesystemExporter, context: ExportContext) -> ArtifactsDelta:
        ...
        manifest, zip_path = self._handle_manifest_and_zip(
            exporter, context, ev_map, selected_articles, metrics_delta
        )
        if isinstance(manifest, dict):
            context.ctx["evidence_manifest"] = manifest
    ...
    def _handle_manifest_and_zip(
        ...
        if isinstance(pre, dict) and ("root" in pre) and ("included" in pre):
            manifest = pre
            zip_path = exporter.zip_from_manifest(context.out_dir, manifest)
        else:
        ...
        metrics_delta["evidence_manifest"] = manifest
