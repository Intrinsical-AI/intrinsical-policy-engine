...
from src.adapters.export.base.evidence.evidence_utils import load_evidence_map as _load_ev_map
...
from src.app.config.constants import EVIDENCE_MANIFEST, EVIDENCE_ZIP, EXPORTS_DIR, ICS_FILE
from src.app.config.context import build_base_context, now_iso_z
from src.domain.exceptions import FingerprintError, InvalidFileError
...
class BaseExporter(ExportHelpers):
    ...
    # Logic moved to src.domain.services.metrics.py
    ...
    def build_evidence_manifest(  # noqa: C901
        ...
        try:
            ...
            manifest: EvidenceManifest = {
                "root": root_rel,
                "root_abs": str(base),  # Absolute path for internal ZIP generation
                "selected_articles": sorted(selected_articles),
    ...
    def zip_selected_evidence(
        ...
        if not man_path.exists():
            self.write_text(man_path, json.dumps(manifest, indent=2, ensure_ascii=False))
        return zpath, cast(dict[str, Any], manifest)

    def zip_from_manifest(self, out_dir: Path, manifest: dict[str, Any]) -> Path | None:
        """Recreate an evidence zip from an on-disk manifest."""
        if not isinstance(manifest, dict):
            return None
        # Prefer root_abs (absolute path) over root (relative) for correct resolution
        root_value = manifest.get("root_abs") or manifest.get("root")
        if not isinstance(root_value, str) or not root_value.strip():
            return None
        try:
            base_root = Path(root_value).resolve()
        except (TypeError, ValueError, OSError):
            return None
