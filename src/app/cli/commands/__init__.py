...
def register_all_commands(subparsers: argparse._SubParsersAction) -> None:
    ...
    - Ops: pdf, package, drift, render
    - Dev: (hidden) build-framework, generate-template, concat
    """
    # Core commands (registered inline)
    from src.app.cli.commands import core

    core.register(subparsers)

    # Validate group
    from src.app.cli.commands import validate

    validate.register(subparsers)

    # Ops group
    from src.app.cli.commands import ops

    ops.register(subparsers)

    # Dev group (hidden from main help)
    from src.app.cli.commands import dev
