...
from src.adapters.export.base.exporters.base_exporter import EvidenceManifest
from src.adapters.export.base.models.registry import (
    canonical_target,
    get_exporter,
    target_name_variants,
)
from src.adapters.export.base.models.shapes import ExportResult
from src.adapters.frameworks.layout_loader import load_framework_layout
from src.adapters.logging import StructuredLogger
from src.adapters.quality.bundle_evidence_validator import CoverageReport
...
from src.app.config.context import build_artifact_context, get_plan_fingerprint
from src.app.export.artifacts import ArtifactsState, ArtifactWriter
from src.app.gating.export_gate import evaluate_bundle_coherence
from src.common.hashing import sha256_directory
from src.common.validation.placeholders import validate_required_context_fields
from src.domain.bundles.predicates import register_core_predicates
from src.domain.exceptions import ExportConfigError, ExportConsistencyError, YAMLLoadError
from src.domain.framework_layout import FrameworkLayout
from src.domain.services.integrity import compute_bundle_hash, compute_plan_hash
...
class ExportOrchestrator:
    ...
    def _prepare_artifacts(self) -> ArtifactsState:
        ...
        if isinstance(wizard_answers, dict) and isinstance(cfg_all, dict):
            if not self.config.include_raw_answers:
                from src.common.sanitization import SanitizationMode, sanitize_payload

                wizard_answers = sanitize_payload(wizard_answers, mode=SanitizationMode.HASH)
    ...
    def _run_targets(  # noqa: C901
        ...
        try:
            adapter = YamlContractsAdapter(strict=self.config.strict)
    ...
    def _run_release_gate(self) -> str | None:
        ...
        from src.app.gating.public_bundle import verify_public_bundle
    ...
    def _ensure_export_context_namespace(self, project_key: str | None = None) -> Plan:
        ...
        try:
            digest = hashlib.sha256(
                str(self.config.contracts_dir).encode(DEFAULT_ENCODING)
            ).hexdigest()
        except (TypeError, ValueError, OSError, UnicodeError):
            digest = "default"

        new_ctx.setdefault("uid_namespace", f"contracts-{digest}")
        # Note: contracts_path omitted to avoid leaking absolute paths into client artifacts.
        # Use bundle_hash in plan.trace for reproducibility instead.
    ...
    def _validate_templates_integrity(self, templates_dir: str) -> str | None:  # noqa: C901
        ...
        from src.app.template_validation import validate_templates
        ...
        try:
            ...
            if integrity_report.errors:
                errors.append(f"Template integrity errors: {len(integrity_report.errors)}")
