...
from intrinsical_policy_engine.adapters.contracts.yaml.contract_validators import (
    ContractBusinessValidator,
)
...
from intrinsical_policy_engine.domain.ports import ContractBundle, ContractsPort
...
def _canonical_bundle_version(
    contracts: dict,
    *,
    legacy_version: str | None = None,
) -> str:
    """Resolve pack identity before falling back to legacy contract versions."""
    metadata = contracts.get("metadata")
...
class YamlContractsAdapter(ContractsPort):
    ...
    Strict mode is explicit. Environment input belongs to an application
    composition root and is never read by this adapter.
    """
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            strict: Whether schema and contract violations fail loading.
            tolerate_questions_errors: Whether malformed optional question
                documents are ignored while loading the rest of the pack.
        """
        ...
        self._strict = strict
        self._tolerate_questions_errors = tolerate_questions_errors

    def _should_tolerate_questions_errors(self) -> bool:
        """Return the explicit question-document tolerance policy."""
        return self._tolerate_questions_errors
    ...
    def load(self, path: str) -> ContractBundle:  # noqa: C901
        ...
        bundle = ContractBundle.from_yaml_dicts(
            ...
            version=_canonical_bundle_version(contracts, legacy_version=meta_version),
            path=str(base_path),
            evidence_map=evidence_map,
    ...
    def _expand_coverage_rules(
        ...
        """
        from intrinsical_policy_engine.domain.bundles.models import BundleNode
        from intrinsical_policy_engine.domain.coverage.context import canonical_node_id
    ...
    def validate(
        ...
        model_errors = self._validate_typed_contracts(contracts, base_path, evidence_map)
        return loading_problems + val_errors + model_errors
    ...
    def validate_detailed(
        ...
        model_errors = self._validate_typed_contracts(contracts, base_path, evidence_map)
        return (loading_problems + val_errors + model_errors), val_problems

    @staticmethod
    def _validate_typed_contracts(
        contracts: dict,
        base_path: Path,
        evidence_map: dict | None,
    ) -> list[str]:
        """Report the same typed-model failures that the assessment load path enforces.

        Schema and business checks operate on raw YAML dictionaries. They cannot
        detect every rejected Pydantic field or shape, so lint must also construct
        the canonical typed bundle instead of claiming a pack is loadable when an
        assessment would reject it later.
        """

        try:
            ContractBundle.from_yaml_dicts(
                flags=contracts.get("flags") or {},
                actions=contracts.get("actions") or {},
                rules=contracts.get("rules") or {},
                articles=contracts.get("articles") or {},
                ...
                risk_config=contracts.get("risk_config") or {},
                runtime=contracts.get("runtime") or {},
                version=_canonical_bundle_version(contracts),
                path=str(base_path),
                evidence_map=evidence_map,
                audit=contracts.get("audit") or {},
                metadata=contracts.get("metadata") or {},
            ...
            return [f"[MODEL][ERROR] Typed contract validation failed: {exc}"]
        return []
