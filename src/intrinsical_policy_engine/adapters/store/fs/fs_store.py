...
from intrinsical_policy_engine.app.config.artifact_names import INDEX_FILE, PLANS_DIR
...
from intrinsical_policy_engine.domain.ports import PlanStorePort
...
class FsPlanStore(PlanStorePort):
    ...
    def save(self, plan_id: str, data: dict[str, Any]) -> None:
        ...
        assessment_timestamp = (
            trace.get("assessment_timestamp") if isinstance(trace, dict) else None
        )
        ts = (
            assessment_timestamp
            if isinstance(assessment_timestamp, str) and assessment_timestamp
