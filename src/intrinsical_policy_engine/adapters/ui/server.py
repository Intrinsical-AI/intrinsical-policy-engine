...
from intrinsical_policy_engine.adapters.ui.templates import PAGE_TEMPLATE, PARTIAL_TEMPLATE
from intrinsical_policy_engine.app.config.artifact_names import (
    ANSWERS_FILE,
    LOCKFILE_NAME,
    LOGS_DIR,
    OUT_DIR,
...
from intrinsical_policy_engine.app.config.environment import UiEnvironment
from intrinsical_policy_engine.app.config.paths import resolve_answers_path
from intrinsical_policy_engine.app.use_cases.ops import render_artifacts as ops_render_artifacts
from intrinsical_policy_engine.app.use_cases.ops import run_export as ops_run_export
from intrinsical_policy_engine.app.use_cases.ops_fs import clean_out_dir as fs_clean_out_dir
from intrinsical_policy_engine.common.cache import CACHE_SIZE_MEDIUM, cached_medium
from intrinsical_policy_engine.common.hashing import sha256_directory
from intrinsical_policy_engine.common.io_safety import acquire_lock, atomic_write_text
from intrinsical_policy_engine.common.io_utils import ensure_dir
from intrinsical_policy_engine.common.json_utils import json_dumps_pretty
from intrinsical_policy_engine.domain.constants import QUESTIONS_FILE
...
from intrinsical_policy_engine.domain.services.questionnaire_engine import (
    _emit_flags_for_question as _emit,
)
...
def create_app(  # noqa: C901
    ...
    *,
    ...
    app.config["UI_CSRF_SECRET"] = config.csrf_secret
    app.config["UI_API_KEY"] = config.api_key
    app.config["UI_VIS_INCREMENTAL"] = config.visibility_incremental
    app.config["UI_VIS_STRICT"] = config.visibility_strict
...
def run_ui_server(
    ...
    *,
    ...
    try:
        ...
        logger.error(
            "[ui] Missing optional dependency: pip install 'intrinsical-policy-engine[ui]'"
        )
        raise

    logger.setLevel(config.log_level)
    app = create_app(contracts_dir, answers_path, config=config)
    app.run(host=host, port=int(port), debug=config.debug)
