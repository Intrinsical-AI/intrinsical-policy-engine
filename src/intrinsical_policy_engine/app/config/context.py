...
from intrinsical_policy_engine.app.config.constants import DEFAULT_ENCODING
from intrinsical_policy_engine.common.constants import (
    CANONICAL_ARTIFACT_SCHEMA_VERSION,
    CANONICAL_ENGINE_NAME,
    CANONICAL_ENGINE_VERSION,
    UNKNOWN_ENGINE_COMMIT,
)
from intrinsical_policy_engine.domain.services.metrics import calculate_techdoc_stats
...
def _inject_meta_and_fingerprint(ctx: dict[str, Any], plan: dict[str, Any]) -> str:
    ...
    trace = _as_mapping(plan.get("trace"))
    ctx["meta"] = {
        "generated_at": generated_at,
        "template_version": "1.0.0",
        "exported_at": exported_at,
        "engine_version": trace.get("engine_version", CANONICAL_ENGINE_VERSION),
        "framework_version": trace.get("framework_version", "unknown"),
        "pack_hash": trace.get("framework_pack_hash", "unknown"),
        "artifact_schema_version": plan.get(
            "artifact_schema_version", CANONICAL_ARTIFACT_SCHEMA_VERSION
        ),
    }
    if isinstance(plan.get("product_name"), str):
        ctx["meta"]["product_name"] = plan["product_name"]
    if isinstance(plan.get("product_version"), str):
        ctx["meta"]["product_version"] = plan["product_version"]
    pack = plan.get("pack")
    if isinstance(pack, dict) and isinstance(pack.get("manifest_timestamp"), str):
        ctx["meta"]["pack_manifest_timestamp"] = pack["manifest_timestamp"]
...
def _apply_engine_defaults(ctx: dict[str, Any], defaults: dict[str, Any]) -> None:
    ...
    # Provenance is engine-owned. A pack or the caller's current Git checkout
    # must never impersonate the distribution that produced an artifact.
    engine["version"] = trace.get("engine_version") or CANONICAL_ENGINE_VERSION
    engine["commit"] = trace.get("engine_commit") or UNKNOWN_ENGINE_COMMIT
...
def get_classification_display(
    outcome: str | list | None,
    *,
    ...
    """
    if display_map is not None and classification_key:
        configured = display_map.get(classification_key)
        if isinstance(configured, dict) and isinstance(configured.get("label"), str):
            label = configured["label"].strip()
            if label:
                severity_value = configured.get("severity", 0)
                severity = (
                    severity_value
                ...
                return {
                    "emoji": str(configured.get("emoji", "")),
                    "label": label,
                    "label_en": str(configured.get("label_en", label)),
                    "css_class": str(configured.get("css_class", "info")),
                    "severity": severity,
                }
...
def _apply_artifact_yaml_defaults(ctx: dict[str, Any], framework_path: Path | None) -> None:
    ...
    display_map = defaults.get("classification_display")
    if isinstance(display_map, dict):
        ctx.setdefault("classification_display", display_map)
...
def _apply_artifact_rendering_context(ctx: dict[str, Any], framework_path: Path | None) -> None:
    ...
    classification_key = ctx.get("risk_tier")
    display_map = ctx.get("classification_display")
    ctx["classification"] = get_classification_display(
        outcome if isinstance(outcome, (str, list)) else "",
        classification_key=classification_key if isinstance(classification_key, str) else None,
        display_map=display_map if isinstance(display_map, dict) else None,
    )
