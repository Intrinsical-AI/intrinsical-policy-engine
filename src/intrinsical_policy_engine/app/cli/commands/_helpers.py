...
from intrinsical_policy_engine.app.config.paths import (
    get_out_dir,
...
from intrinsical_policy_engine.common.hashing import compute_framework_pack_hashes, sha256_directory
...
def load_bundle_and_answers(
    ...
    requested_outdir = resolve_out_dir(getattr(args, "out", "out"))
    validate_export_output_boundary(contracts_dir, requested_outdir)

    # Compatibility and license metadata are hard pack boundaries for every
    # CLI operation, not just the typed embedding facade. Resolve them before
    # creating outputs, log files, or debug traces.
...
def run_assessment(
    ...
    environment = getattr(args, "_ipe_environment", None)
    if bool(getattr(environment, "demo_mode", False)):
        plan["demo_mode"] = True

    # Debug trace
    if getattr(args, "debug", False):
        from intrinsical_policy_engine.app.config.paths import get_out_dir
...
def check_ci_compliance(plan: Plan, strict: bool) -> int:
    ...
    """
    from intrinsical_policy_engine.app.cli.commands.ci_runner import CIRunner
