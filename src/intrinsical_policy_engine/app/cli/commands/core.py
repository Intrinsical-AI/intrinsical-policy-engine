...
def _register_lint(subparsers: argparse._SubParsersAction) -> None:
    ...
    strict_group.add_argument(
        ...
        dest="strict",
        action="store_false",
        help="Use tolerant contract validation",
    )
...
def _handle_lint(args: Namespace) -> int:
    ...
    from intrinsical_policy_engine.app.config.paths import resolve_contracts_path
    from intrinsical_policy_engine.app.use_cases import ops

    environment = getattr(args, "_ipe_environment", None)
    adapter = YamlContractsAdapter(
        strict=bool(args.strict),
        tolerate_questions_errors=bool(getattr(environment, "tolerate_questions_errors", False)),
    )
    contracts_dir = resolve_contracts_path(args.contracts)

    try:
        FilesystemPackProvider().resolve(contracts_dir)
    except ValueError as exc:
        sys.stdout.write(json.dumps({"status": "FAIL", "problems": [str(exc)]}, indent=2) + "\n")
        return 1
...
def _handle_assess(args: Namespace) -> int:
    ...
    """
    from intrinsical_policy_engine.app.cli.commands._helpers import (
        check_ci_compliance,
...
def _handle_export(args: Namespace) -> int:
    ...
    # Reject an invalid release policy before loading untrusted packs, creating
    # log paths, or writing debug assessment traces.
    strict = bool(getattr(args, "strict", False))
    strict_templates = bool(getattr(args, "strict_templates", False) or strict)
    environment = getattr(args, "_ipe_environment", None)
    allow_incomplete_coverage = getattr(environment, "allow_incomplete_coverage", None)
    gate_policy = ExportGatePolicy(
        ...
        skip_gpg_signing=bool(getattr(environment, "skip_gpg_signing", False)),
    )
    preflight_gate = evaluate_export_gate(ExportGateFacts(), gate_policy)
    if preflight_gate.blocked:
        sys.stderr.write(
            "[export] Invalid release policy: " + ", ".join(preflight_gate.blockers) + "\n"
        )
        return 1

    # Load bundle and answers only after the side-effect-free preflight.
    bundle, raw_answers, outdir, logger = load_bundle_and_answers(args)
    contracts_dir = resolve_contracts_path(args.contracts)

    # Run assessment first
    plan = run_assessment(bundle, raw_answers, contracts_dir, logger, args)
    descriptor = FilesystemPackProvider().resolve(contracts_dir)
    plan["artifact_schema_version"] = CANONICAL_ARTIFACT_SCHEMA_VERSION
    plan["pack"] = {
        "id": descriptor.id,
        "version": descriptor.version,
        "manifest_timestamp": descriptor.manifest_timestamp,
    ...
    result = ops.run_export(
        ...
        skip_gpg_signing=gate_policy.skip_gpg_signing,
    )

    gate_outcome = evaluate_export_gate(
        ExportGateFacts(
            pre_artifact_error=bool(result.pre_artifact_error),
            quality_gating_error=bool(result.quality_gating_error),
            release_gate_error=bool(result.release_gate_error),
            bundle_coherence_error=bool(result.bundle_coherence_error),
            target_errors=tuple(result.target_errors),
        ),
        gate_policy,
    )
...
def _handle_seal(args: Namespace) -> int:
    ...
    environment = getattr(args, "_ipe_environment", None)
    sign = not args.no_sign and not bool(getattr(environment, "skip_gpg_signing", False))
    ...
    except Exception as exc:  # noqa: BLE001 - CLI fault boundary
        sys.stderr.write(f"Seal failed: {exc}\n")
        return 1
...
def _handle_ui(args: Namespace) -> int:
    ...
    from intrinsical_policy_engine.api.packs import FilesystemPackProvider
    from intrinsical_policy_engine.app.config.environment import load_ui_environment
    from intrinsical_policy_engine.app.config.paths import resolve_contracts_path

    contracts_dir = resolve_contracts_path(args.contracts)
    FilesystemPackProvider().resolve(contracts_dir)

    # Keep the complete optional UI path behind one friendly dependency error.
    try:
        from intrinsical_policy_engine.adapters.ui.server import run_ui_server
        ...
        run_ui_server(
            ...
            host=args.host,
            port=args.port,
            config=load_ui_environment(),
        )
    except ImportError:
        sys.stderr.write(
            "UI requires optional dependencies. "
            "Install with: pip install 'intrinsical-policy-engine[ui]'\n"
        )
        return 1
    return 0
...
def _handle_wizard(args: Namespace) -> int:
    ...
    from intrinsical_policy_engine.app.cli.wizard import ComplianceWizard
    from intrinsical_policy_engine.app.config.constants import DEFAULT_ENCODING, WIZARD_ANSWERS_JSON
    from intrinsical_policy_engine.app.config.paths import resolve_contracts_path
...
def _handle_inspect(args: Namespace) -> int:
    ...
    from intrinsical_policy_engine.api.packs import FilesystemPackProvider
    from intrinsical_policy_engine.app.cli.commands.inspect import inspect_rule
    from intrinsical_policy_engine.app.config.paths import resolve_contracts_path
...
def _add_common_args(parser: argparse.ArgumentParser) -> None:
    ...
    strict_group.add_argument(
        ...
        dest="strict",
        action="store_false",
        help="Use tolerant contract and export validation",
    )
    parser.set_defaults(strict=None)
    parser.add_argument(
        "--strict-templates",
