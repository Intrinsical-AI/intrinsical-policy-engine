...
class MockFileSystem:
    ...
    def __init__(
        ...
        self._dirs: set[str] = existing_dirs or set()
    ...
    def exists(self, path: Path) -> bool:
        ...
        return any(
            path_str.endswith(item) or item.endswith(str(path.name))
            for item in self._files | self._dirs
        )

    def is_dir(self, path: Path) -> bool:
        """Return whether the mock path was declared as a directory."""
        path_str = str(path)
        return any(
            path_str == item or path_str.endswith(item) or item.endswith(str(path.name))
