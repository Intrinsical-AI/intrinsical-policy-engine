...
_VOLATILE_PLAN_FIELDS = frozenset(
    {
        ...
        "artifact_schema_version",  # Export envelope version, not assessment semantics
        "pack",  # Pack distribution provenance added at export time
        "product",  # Optional product-shell provenance added at export time
        "product_name",  # Flattened template view of product provenance
        "product_version",  # Flattened template view of product provenance
...
def compute_plan_hash(plan_data: dict[str, Any]) -> str:
    ...
    Note:
        Volatile fields excluded: assessment_timestamp, export_context, export
        provenance (artifact schema and product identity), audit, system,
        wizard_answers, plan_hash, and trace.answers_raw.
    """

    def _strip_volatile(obj: dict, *, top_level: bool = False) -> dict:
        ...
        for k, v in obj.items():
            if top_level and k in _VOLATILE_PLAN_FIELDS:
                continue
            if top_level and k == "trace" and isinstance(v, dict):
                ...
                # A nested field can legitimately share a name with an
                # export-only top-level key (for example an action's
                # ``product`` constraint). It remains semantic.
                result[k] = _strip_volatile(v, top_level=False)
            else:
                result[k] = v
        return result

    clean_plan = _strip_volatile(plan_data, top_level=True)
