...
from intrinsical_policy_engine.common.cache import cached_large
from intrinsical_policy_engine.domain.exceptions import RuleEvaluationError, RuleParseError
from intrinsical_policy_engine.domain.types import ASTNode, Flag, PackId
...
class _ParsedWhenString(NamedTuple):
    """Cached string parse result and its maximum relative nesting depth."""
...
@cached_large
def _parse_when_string_cached(s: str) -> _ParsedWhenString:
    """Parse a string once and cache its AST plus relative nesting depth."""
    if not s.strip():
        raise RuleParseError(expression="<empty>", reason="Empty expression")
    try:
        parser = Parser(s)
        ast = parser.parse()
        return _ParsedWhenString(ast=ast, relative_depth=parser.max_depth)
...
def parse_when(obj: dict[str, Any] | str | None, _depth: int = 0) -> ASTNode:
    ...
    if isinstance(obj, str):
        parsed = _parse_when_string_cached(obj)
        if _depth + parsed.relative_depth > MAX_PARSE_DEPTH:
            raise RuleParseError(expression=obj[:100], reason="Expression too deeply nested")
        return parsed.ast
